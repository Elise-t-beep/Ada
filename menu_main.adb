...
--------------------------------------------------------------------------------------------------
   ...
   BEGIN
      ...
      AFFICHage_prefixe(A);
      LOOP
         Menu_Premier_Choix(Initialisation_id,Listed,document,L,Date_Jour,D,Demande,Login,Empreinte,Fonction,Cpt,id,pers,choix,a,pat);
         IF Cpt = 3 THEN
...
PROCEDURE Menu_Secretaire (A: IN OUT T_Arbre;ListeD: IN OUT T_Pteurdoc;D: OUT T_Document;Initialisation_Id:in out Integer;Date_Jour: OUT T_Date;Pers:OUT T_Personnel;L:T_Pteurpers;Id: out t_pers)IS
     rep:integer;CATEGORIE:t_nature;
     BEGIN
      LOOP
         LOOP
